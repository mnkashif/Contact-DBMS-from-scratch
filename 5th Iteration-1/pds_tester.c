...
void process_line( char *test_case )
{
	...
	char phone_num[10];
	int contact_id, status, rec_size, expected_status, expected_read_count, actual_read_count;
	...
	if( !strcmp(command,"OPEN") ){
		...
		sscanf(param2, "%d", &expected_read_count);
		testContact.contact_id = -1;
		int actual_read_count = 0;
		status = search_contact_by_phone( phone_num, &testContact, &actual_read_count );
		if(status == PDS_SUCCESS)
			status = CONTACT_SUCCESS;
		else
			status = CONTACT_FAILURE;
		if( status != expected_status ){
			...
			if( expected_status == 0 ){
				if (testContact.contact_id == contact_id && 
					strcmp(testContact.contact_name, expected_name) == 0 &&
					strcmp(testContact.phone, expected_phone) == 0 ){
						if( expected_read_count == actual_read_count ){
							TREPORT("PASS", "");
						}
						else{
							sprintf(info,"Num I/O not matching for %d... Expected:%d Got:%d\n",
								contact_id, expected_read_count, actual_read_count
							);
							TREPORT("FAIL", info);
						}
				}
